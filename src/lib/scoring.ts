// ============================================================
// ОСКОРБЛЯТОР — Scoring Engine v2
// ============================================================

type WordEntry = {
  word: string;
  score: number;   // 1–10 (сила обидности)
  category: string;
};

// ── База слов ────────────────────────────────────────────────
// Русский мат и оскорбления (ядро)
const RU_WORDS: WordEntry[] = [
  // Категория: прямой мат (score 7–10)
  { word: "хуй",        score: 9,  category: "mat" },
  { word: "хуйня",      score: 8,  category: "mat" },
  { word: "хуёво",      score: 7,  category: "mat" },
  { word: "хуёвый",     score: 7,  category: "mat" },
  { word: "пизда",      score: 9,  category: "mat" },
  { word: "пиздёж",     score: 7,  category: "mat" },
  { word: "пиздатый",   score: 6,  category: "mat" },
  { word: "пиздец",     score: 8,  category: "mat" },
  { word: "ёбаный",     score: 8,  category: "mat" },
  { word: "ёбнутый",    score: 8,  category: "mat" },
  { word: "ёб",         score: 8,  category: "mat" },
  { word: "блядь",      score: 9,  category: "mat" },
  { word: "блядина",    score: 9,  category: "mat" },
  { word: "шлюха",      score: 8,  category: "mat" },
  { word: "сука",       score: 8,  category: "mat" },
  { word: "сучка",      score: 7,  category: "mat" },
  { word: "сучара",     score: 8,  category: "mat" },
  { word: "пёс",        score: 6,  category: "mat" },
  { word: "пёздрик",    score: 7,  category: "mat" },
  { word: "нахуй",      score: 8,  category: "mat" },
  { word: "похуй",      score: 7,  category: "mat" },
  { word: "охуел",      score: 8,  category: "mat" },
  { word: "охуенный",   score: 8,  category: "mat" },
  { word: "охуительный",score: 8,  category: "mat" },
  { word: "мудак",      score: 9,  category: "mat" },
  { word: "мудила",     score: 9,  category: "mat" },
  { word: "мудозвон",   score: 9,  category: "mat" },
  { word: "мудень",     score: 8,  category: "mat" },
  { word: "муда",       score: 7,  category: "mat" },
  { word: "пидор",      score: 10, category: "mat" },
  { word: "пидарас",    score: 10, category: "mat" },
  { word: "педик",      score: 9,  category: "mat" },
  { word: "ёбаный рот", score: 10, category: "mat" },
  { word: "залупа",     score: 9,  category: "mat" },
  { word: "ёб твою мать", score: 10, category: "mat" },
  { word: "иди нахуй",  score: 9,  category: "mat" },
  { word: "нафига",     score: 4,  category: "mat" },
  { word: "задрот",     score: 7,  category: "mat" },
  { word: "задница",    score: 5,  category: "mat" },
  { word: "жопа",       score: 6,  category: "mat" },
  { word: "жопоголовый",score: 8,  category: "mat" },
  { word: "жополиз",    score: 8,  category: "mat" },
  { word: "жопошник",   score: 8,  category: "mat" },
  { word: "ублюдок",    score: 9,  category: "mat" },
  { word: "ублюдина",   score: 9,  category: "mat" },
  { word: "выблядок",   score: 10, category: "mat" },
  { word: "ёбнул",      score: 7,  category: "mat" },
  { word: "долбоёб",    score: 9,  category: "mat" },
  { word: "долбанутый", score: 8,  category: "mat" },
  { word: "долбак",     score: 8,  category: "mat" },
  { word: "мразь",      score: 9,  category: "mat" },
  { word: "мразота",    score: 9,  category: "mat" },
  { word: "тварь",      score: 8,  category: "mat" },
  { word: "тварина",    score: 8,  category: "mat" },
  { word: "скотина",    score: 7,  category: "mat" },
  { word: "скот",       score: 7,  category: "mat" },
  { word: "падла",      score: 8,  category: "mat" },
  { word: "падаль",     score: 9,  category: "mat" },
  { word: "падонок",    score: 8,  category: "mat" },
  { word: "гандон",     score: 9,  category: "mat" },
  { word: "гандончик",  score: 7,  category: "mat" },
  { word: "шалава",     score: 8,  category: "mat" },
  { word: "шлюшка",     score: 7,  category: "mat" },
  { word: "потаскуха",  score: 8,  category: "mat" },
  { word: "курва",      score: 8,  category: "mat" },
  { word: "кобель",     score: 6,  category: "mat" },
  { word: "сволочь",    score: 7,  category: "mat" },
  { word: "мерзавец",   score: 7,  category: "mat" },
  { word: "негодяй",    score: 6,  category: "mat" },
  { word: "подонок",    score: 8,  category: "mat" },
  { word: "урод",       score: 7,  category: "mat" },
  { word: "уродина",    score: 7,  category: "mat" },
  { word: "уродец",     score: 6,  category: "mat" },

  // Категория: интеллект (score 4–8)
  { word: "дебил",      score: 8,  category: "intel" },
  { word: "дебилоид",   score: 8,  category: "intel" },
  { word: "дебилушка",  score: 7,  category: "intel" },
  { word: "идиот",      score: 7,  category: "intel" },
  { word: "идиотина",   score: 7,  category: "intel" },
  { word: "кретин",     score: 7,  category: "intel" },
  { word: "кретинос",   score: 7,  category: "intel" },
  { word: "тупица",     score: 6,  category: "intel" },
  { word: "тупорылый",  score: 7,  category: "intel" },
  { word: "тупак",      score: 6,  category: "intel" },
  { word: "тупень",     score: 6,  category: "intel" },
  { word: "тупоголовый",score: 7,  category: "intel" },
  { word: "безмозглый", score: 6,  category: "intel" },
  { word: "безмозглина",score: 6,  category: "intel" },
  { word: "дурак",      score: 5,  category: "intel" },
  { word: "дуралей",    score: 5,  category: "intel" },
  { word: "дурень",     score: 5,  category: "intel" },
  { word: "дурачок",    score: 5,  category: "intel" },
  { word: "придурок",   score: 7,  category: "intel" },
  { word: "придурошный",score: 7,  category: "intel" },
  { word: "балбес",     score: 5,  category: "intel" },
  { word: "болван",     score: 5,  category: "intel" },
  { word: "олух",       score: 5,  category: "intel" },
  { word: "остолоп",    score: 5,  category: "intel" },
  { word: "оболтус",    score: 5,  category: "intel" },
  { word: "бестолочь",  score: 5,  category: "intel" },
  { word: "недоумок",   score: 7,  category: "intel" },
  { word: "недоросль",  score: 5,  category: "intel" },
  { word: "лопух",      score: 4,  category: "intel" },
  { word: "лох",        score: 6,  category: "intel" },
  { word: "лошара",     score: 6,  category: "intel" },
  { word: "чурбан",     score: 5,  category: "intel" },
  { word: "чучело",     score: 5,  category: "intel" },
  { word: "пень",       score: 4,  category: "intel" },
  { word: "пенёк",      score: 4,  category: "intel" },
  { word: "баран",      score: 6,  category: "intel" },
  { word: "баранина",   score: 6,  category: "intel" },
  { word: "кретинизм",  score: 6,  category: "intel" },
  { word: "слабоумный", score: 7,  category: "intel" },
  { word: "умственно отсталый", score: 7, category: "intel" },
  { word: "имбецил",    score: 8,  category: "intel" },
  { word: "аутист",     score: 6,  category: "intel" },
  { word: "зомби",      score: 5,  category: "intel" },
  { word: "овощ",       score: 6,  category: "intel" },
  { word: "овощной",    score: 5,  category: "intel" },

  // Категория: внешность (score 4–7)
  { word: "страхолюдина", score: 7, category: "appearance" },
  { word: "страшила",   score: 6,  category: "appearance" },
  { word: "крокодил",   score: 6,  category: "appearance" },
  { word: "бегемот",    score: 6,  category: "appearance" },
  { word: "жирдяй",     score: 6,  category: "appearance" },
  { word: "жиртрест",   score: 7,  category: "appearance" },
  { word: "жирный",     score: 5,  category: "appearance" },
  { word: "толстый",    score: 4,  category: "appearance" },
  { word: "толстопузый",score: 5,  category: "appearance" },
  { word: "харя",       score: 7,  category: "appearance" },
  { word: "рожа",       score: 6,  category: "appearance" },
  { word: "морда",      score: 6,  category: "appearance" },
  { word: "мордоворот", score: 7,  category: "appearance" },
  { word: "рыло",       score: 6,  category: "appearance" },
  { word: "пятак",      score: 5,  category: "appearance" },
  { word: "свинья",     score: 7,  category: "appearance" },
  { word: "свинтус",    score: 7,  category: "appearance" },
  { word: "свинорылый", score: 8,  category: "appearance" },

  // Категория: происхождение/семья (score 6–10)
  { word: "ублюдок",    score: 9,  category: "family" },
  { word: "байстрюк",   score: 8,  category: "family" },
  { word: "незаконнорождённый", score: 7, category: "family" },
  { word: "твоя мать",  score: 7,  category: "family" },
  { word: "мамаша",     score: 5,  category: "family" },
  { word: "папаша",     score: 5,  category: "family" },
  { word: "мразь рождённая", score: 9, category: "family" },

  // Категория: ничтожество / бесполезность (score 5–9)
  { word: "ничтожество",score: 8,  category: "worthless" },
  { word: "ничтожина",  score: 8,  category: "worthless" },
  { word: "ноль",       score: 5,  category: "worthless" },
  { word: "нулина",     score: 5,  category: "worthless" },
  { word: "пустышка",   score: 5,  category: "worthless" },
  { word: "пустозвон",  score: 6,  category: "worthless" },
  { word: "никчёмный",  score: 7,  category: "worthless" },
  { word: "никчёмность",score: 7,  category: "worthless" },
  { word: "бесполезный",score: 6,  category: "worthless" },
  { word: "бесполезность", score: 6, category: "worthless" },
  { word: "недоделанный", score: 7, category: "worthless" },
  { word: "недоразумение", score: 7, category: "worthless" },
  { word: "недочеловек",score: 9,  category: "worthless" },
  { word: "недоросток", score: 6,  category: "worthless" },
  { word: "неудачник",  score: 6,  category: "worthless" },
  { word: "неудачница", score: 6,  category: "worthless" },
  { word: "лузер",      score: 6,  category: "worthless" },
  { word: "лузерок",    score: 6,  category: "worthless" },
  { word: "позор",      score: 7,  category: "worthless" },
  { word: "позорище",   score: 8,  category: "worthless" },
  { word: "посмешище",  score: 7,  category: "worthless" },
  { word: "чмо",        score: 8,  category: "worthless" },
  { word: "чмошник",    score: 8,  category: "worthless" },
  { word: "чморик",     score: 7,  category: "worthless" },
  { word: "шлак",       score: 6,  category: "worthless" },
  { word: "отброс",     score: 8,  category: "worthless" },
  { word: "отброски",   score: 8,  category: "worthless" },
  { word: "мусор",      score: 6,  category: "worthless" },
  { word: "мусорина",   score: 7,  category: "worthless" },
  { word: "дно",        score: 7,  category: "worthless" },
  { word: "шелупонь",   score: 7,  category: "worthless" },
  { word: "гниль",      score: 7,  category: "worthless" },
  { word: "гнида",      score: 8,  category: "worthless" },
  { word: "гнидарь",    score: 8,  category: "worthless" },
  { word: "вошь",       score: 7,  category: "worthless" },
  { word: "паразит",    score: 7,  category: "worthless" },
  { word: "паразитина", score: 8,  category: "worthless" },
  { word: "клоп",       score: 6,  category: "worthless" },
  { word: "тараканина", score: 6,  category: "worthless" },
  { word: "слизняк",    score: 6,  category: "worthless" },
  { word: "слизень",    score: 6,  category: "worthless" },
  { word: "глист",      score: 7,  category: "worthless" },
  { word: "глистина",   score: 7,  category: "worthless" },
  { word: "червяк",     score: 5,  category: "worthless" },
  { word: "червяком",   score: 5,  category: "worthless" },
  { word: "плесень",    score: 7,  category: "worthless" },

  // Категория: трус / предатель (score 5–8)
  { word: "трус",       score: 6,  category: "coward" },
  { word: "трусишка",   score: 5,  category: "coward" },
  { word: "трусило",    score: 6,  category: "coward" },
  { word: "шкура",      score: 7,  category: "coward" },
  { word: "предатель",  score: 7,  category: "coward" },
  { word: "предательская крыса", score: 8, category: "coward" },
  { word: "крыса",      score: 7,  category: "coward" },
  { word: "стукач",     score: 7,  category: "coward" },
  { word: "доносчик",   score: 7,  category: "coward" },
  { word: "слизняк",    score: 6,  category: "coward" },

  // Повторяющиеся эмоциональные усилители
  { word: "проклятый",  score: 5,  category: "amplifier" },
  { word: "проклятая",  score: 5,  category: "amplifier" },
  { word: "чёртов",     score: 5,  category: "amplifier" },
  { word: "чёртова",    score: 5,  category: "amplifier" },
  { word: "сраный",     score: 6,  category: "amplifier" },
  { word: "сраная",     score: 6,  category: "amplifier" },
  { word: "поганый",    score: 6,  category: "amplifier" },
  { word: "поганая",    score: 6,  category: "amplifier" },
  { word: "гнусный",    score: 6,  category: "amplifier" },
  { word: "мерзкий",    score: 6,  category: "amplifier" },
  { word: "отвратительный", score: 6, category: "amplifier" },
  { word: "омерзительный",  score: 7, category: "amplifier" },
  { word: "жалкий",     score: 6,  category: "amplifier" },
  { word: "жалкая",     score: 6,  category: "amplifier" },
];

// Английские оскорбления
const EN_WORDS: WordEntry[] = [
  { word: "fuck",        score: 8,  category: "mat" },
  { word: "fucking",     score: 8,  category: "mat" },
  { word: "fucked",      score: 7,  category: "mat" },
  { word: "fucker",      score: 8,  category: "mat" },
  { word: "motherfucker",score: 10, category: "mat" },
  { word: "shit",        score: 6,  category: "mat" },
  { word: "shithead",    score: 8,  category: "mat" },
  { word: "bullshit",    score: 6,  category: "mat" },
  { word: "asshole",     score: 8,  category: "mat" },
  { word: "asshat",      score: 7,  category: "mat" },
  { word: "bastard",     score: 8,  category: "mat" },
  { word: "bitch",       score: 8,  category: "mat" },
  { word: "son of a bitch", score: 9, category: "mat" },
  { word: "cunt",        score: 10, category: "mat" },
  { word: "dick",        score: 7,  category: "mat" },
  { word: "dickhead",    score: 8,  category: "mat" },
  { word: "cock",        score: 7,  category: "mat" },
  { word: "damn",        score: 4,  category: "mat" },
  { word: "hell",        score: 3,  category: "mat" },
  { word: "idiot",       score: 6,  category: "intel" },
  { word: "moron",       score: 7,  category: "intel" },
  { word: "imbecile",    score: 7,  category: "intel" },
  { word: "retard",      score: 8,  category: "intel" },
  { word: "stupid",      score: 5,  category: "intel" },
  { word: "dumbass",     score: 7,  category: "intel" },
  { word: "dumb",        score: 5,  category: "intel" },
  { word: "brainless",   score: 6,  category: "intel" },
  { word: "loser",       score: 6,  category: "worthless" },
  { word: "pathetic",    score: 6,  category: "worthless" },
  { word: "worthless",   score: 7,  category: "worthless" },
  { word: "useless",     score: 6,  category: "worthless" },
  { word: "trash",       score: 6,  category: "worthless" },
  { word: "garbage",     score: 6,  category: "worthless" },
  { word: "scum",        score: 8,  category: "worthless" },
  { word: "scumbag",     score: 8,  category: "worthless" },
  { word: "pig",         score: 6,  category: "appearance" },
  { word: "ugly",        score: 5,  category: "appearance" },
  { word: "fat",         score: 4,  category: "appearance" },
  { word: "coward",      score: 6,  category: "coward" },
  { word: "wimp",        score: 5,  category: "coward" },
  { word: "rat",         score: 7,  category: "coward" },
  { word: "snake",       score: 6,  category: "coward" },
];

const ALL_WORDS = [...RU_WORDS, ...EN_WORDS];

// ── Главная функция оценки v3 ────────────────────────────────
// Ключевая философия:
//   • Структура предложения — ГЛАВНЫЙ фактор (до 60 очков)
//   • Оскорбительность — ВТОРОСТЕПЕННЫЙ фактор (до 35 очков) с сильным убыванием
//   • Списки матных слов без предложений — штрафуются
//   • Связный развёрнутый оскорбительный текст — максимальный балл
export function scoreInsult(insult: string): number {
  if (!insult || insult.trim().length < 2) return 0;
  const text = insult.trim();
  const lower = text.toLowerCase();

  // ── 1. ОСКОРБИТЕЛЬНОСТЬ (0–35 очков) — второстепенный фактор ──
  // Сильное убывание: первое слово важно, остальные почти не считаются.
  // Цель: список из 10 матных слов не должен давать больше, чем 1 сильное
  // слово в структурированном предложении.
  const matched: WordEntry[] = [];
  for (const entry of ALL_WORDS) {
    if (lower.includes(entry.word.toLowerCase())) {
      matched.push(entry);
    }
  }
  const sortedWords = [...matched].sort((a, b) => b.score - a.score);
  let offScore = 0;
  sortedWords.forEach((w, i) => {
    const mult = i === 0 ? 2.0    // топ-слово: максимальный вес
               : i === 1 ? 0.8    // второе:  небольшой бонус
               : i === 2 ? 0.3    // третье:  минимальный
               :           0.08;  // остальные: почти ноль
    offScore += w.score * mult;
  });
  offScore = Math.min(35, offScore);

  // ── 2. ЛИНГВИСТИЧЕСКАЯ СТРУКТУРА (0–60 очков) — главный фактор ──
  // Оцениваются признаки грамматически связного предложения.
  //
  // ВАЖНО: JavaScript \b не работает с кириллицей (считает её \W),
  // поэтому используем (?<![а-яёА-ЯЁ]) / (?![а-яёА-ЯЁ]) вместо \b.
  const CY  = "[а-яёА-ЯЁ]";
  const NB  = `(?<!${CY})`;   // не предшествует кириллица
  const NA  = `(?!${CY})`;    // не следует кириллица
  // Токены для слов-триггеров
  const tok = (w: string) => new RegExp(`${NB}${w}${NA}`, "i");
  // Слова-токены в альтернации
  const any = (ws: string) => new RegExp(`${NB}(${ws})${NA}`, "i");

  let strScore = 0;

  // 2a. Личное обращение — подлежащее / адресат
  if (any("ты|вы|тебя|тебе|твой|твоя|твоё|твоего|твоему|твоих|ваш|вашу|вашего|вам|вас").test(lower) ||
      /\b(you|your|yours|yourself)\b/i.test(lower)) {
    strScore += 5;
  }

  // 2b. Спрягаемый глагол (сказуемое) — самый важный признак предложения.
  //     Инфинитивы-восклицания ("ебать!") намеренно не считаются.
  const w = lower.split(/[^а-яёa-zA-Z]+/).filter(Boolean); // слова текста
  const VERB_EXACT = new Set([
    "является","есть","был","была","было","были","будет","будешь",
    "стал","стала","стали","может","можешь","должен","должна",
    "хочет","хочешь","знает","знаешь","думает","думаешь","плачет",
    "плачешь","идёт","идёшь","делает","делаешь","говорит","говоришь",
    "имеет","видит","видишь","слышит","несёт","берёт","живёт",
    "рассматривает","считает","падает","смотрит","смотришь",
    "стоит","стоишь","кажется","являешься","представляет",
    // английские
    "are","is","was","were","have","has","makes","does","did",
    "look","looks","seems","feel","feels","think","thinks",
  ]);
  const VERB_SUFFIX = /(?:ешь|ёшь|ишь|ает|яет|ует|юет|ают|яют|уют|ют|ят|ут|ат|тся|ться)$/;
  const hasConjugatedVerb = w.some(wd => VERB_EXACT.has(wd) || (wd.length >= 4 && VERB_SUFFIX.test(wd)));
  if (hasConjugatedVerb) strScore += 10;

  // 2c. Сравнительная конструкция (как / словно / будто / точно)
  if (any("как|словно|будто|точно|подобно").test(lower)) strScore += 8;

  // 2d. Причинно-следственная связь
  if (/потому что|поскольку|так как|раз уж|ведь|ибо/i.test(lower)) strScore += 7;

  // 2e. Гипербола: «настолько/такой ... что»
  if (/настолько.{2,60}(что|чтобы)/i.test(lower) || /столь .{2,60}(что|чтобы)/i.test(lower)) {
    strScore += 11;
  } else if (/(такой|такая|такое|такие).{2,60}(что|чтобы)/i.test(lower)) {
    strScore += 9;
  }

  // 2f. Усилительная частица / категорическое отрицание
  if (any("даже|никогда|нисколько|ни капли").test(lower) || /ни разу|ни один/i.test(lower)) {
    strScore += 6;
  }

  // 2g. Семейный удар
  if (/твоя мать|твой отец|твои родители|мамаша|папаша/i.test(lower)) strScore += 8;

  // 2h. Философское / метаобразное наблюдение
  if (/живое доказательство|живой пример|биологическая ошибка|ошибка природы|ошибка эволюции|квинтэссенция|воплощение|олицетворение/i.test(lower)) {
    strScore += 10;
  }

  // 2i. Придаточное определительное (который / чей)
  if (any("который|которая|которое|которые|которого|которому|которым|чей|чья|чьи|чьего").test(lower)) {
    strScore += 6;
  }

  // 2j. Цепочка прилагательных
  if (/[а-яё]{3,}(ый|ий|ой|ая|яя|ое|ее|ые|ие)\s+[а-яё]{3,}(ый|ий|ой|ая|яя|ое|ее|ые|ие)/i.test(lower)) {
    strScore += 4;
  }

  // 2k. Уступительная / условная конструкция
  if (/хотя |несмотря на |даже если |если бы |при всём /i.test(lower)) strScore += 5;

  // 2l. Диапазон-оскорбление (от ... до)
  if (/от .{2,20} до /i.test(lower)) strScore += 5;

  strScore = Math.min(60, strScore);

  // ── 3. ШТРАФ ЗА СПИСОК СЛОВ (0–25 очков вычитается) ──
  // Если текст — это просто перечисление матных слов через запятую
  // без предикативной структуры — штрафуем.
  const commaCount = (text.match(/,/g) || []).length;
  let listPenalty = 0;
  if (commaCount >= 3 && strScore < 22) {
    // Считаем среднюю длину сегмента между знаками препинания
    const segs = text.split(/[,;!?]+/).filter(s => s.trim().length > 0);
    const avgSegLen = segs.reduce((sum, seg) => sum + seg.trim().replace(/\s+/g, " ").length, 0) / segs.length;
    if (avgSegLen < 15) {
      // Короткие сегменты + много запятых + слабая структура = список слов
      listPenalty = Math.min(25, Math.round((commaCount - 2) * 3 + Math.max(0, 13 - avgSegLen) * 1.5));
    }
  }

  // ── 4. СТИЛИСТИЧЕСКИЕ БОНУСЫ (0–15 очков) ──
  const len = text.length;
  const lengthBonus = len < 10  ? -5
                    : len < 25  ? 1
                    : len < 50  ? 3
                    : len < 150 ? 5
                    : len < 300 ? 4
                    : 2; // очень длинный текст теряет концентрацию

  // Восклицательные / вопросительные знаки — эмоциональный накал
  const punctCount = (text.match(/[!?]/g) || []).length;
  const punctBonus  = Math.min(4, Math.floor(punctCount * 1.5));

  // КАПС-слова: 1–2 слова = акцент, много слов = весь текст в крике
  const allWords    = text.split(/\s+/);
  const capsCount   = allWords.filter(w => w.length > 2 && w === w.toUpperCase() && /[А-ЯA-Z]/.test(w)).length;
  let capsBonus = 0;
  if (capsCount >= Math.ceil(allWords.length * 0.5) && capsCount >= 3) {
    capsBonus = 7; // весь текст в КАПСЕ — эпическая подача
  } else {
    capsBonus = Math.min(6, capsCount * 2);
  }

  // ── 5. ДЕТЕРМИНИРОВАННАЯ ДИСПЕРСИЯ (±7) ──
  let hash = 0;
  for (let i = 0; i < text.length; i++) {
    hash = ((hash << 5) - hash + text.charCodeAt(i)) | 0;
  }
  const variance = (Math.abs(hash) % 15) - 7;

  const total = offScore + strScore - listPenalty + lengthBonus + punctBonus + capsBonus + variance;
  return Math.max(1, Math.min(100, Math.round(total)));
}

// ── Объяснение оценки (для дебага / будущих фич) ────────────
export function explainScore(insult: string) {
  const lower = insult.toLowerCase();
  const matched = ALL_WORDS.filter(e => lower.includes(e.word.toLowerCase()));
  const categories = [...new Set(matched.map(w => w.category))];
  const topWords   = [...matched].sort((a, b) => b.score - a.score).slice(0, 5);
  return { topWords, categories, score: scoreInsult(insult) };
}

// ── Жёсткие оскорбления бота ────────────────────────────────
// Сгруппированы по силе, чтобы бот мог выбирать уровень

const BOT_TIER_1: string[] = [ // score ~20–35
  "Ты немного неприятный человечек.",
  "Твои слова такие же пустые, как твоя голова.",
  "Ты раздражаешь меня своим существованием.",
  "С тобой неприятно иметь дело.",
  "Ты лузер районного масштаба.",
  "Твоё мнение не имеет никакой ценности.",
  "Общаться с тобой — всё равно что смотреть тест-полосу на телевизоре.",
  "Ты настолько незначителен, что даже тень тебя не уважает.",
  "Твои достижения можно описать одним словом: никакие.",
  "Скучнее тебя только наблюдение за сохнущей краской.",
  "Ты — человек без особых примет в буквальном смысле.",
  "Твоё присутствие немного портит атмосферу в комнате.",
  "Ты похож на фоновый персонаж из чужой жизни.",
  "Твоя харизма — это ноль с претензией на единицу.",
  "С тобой не интересно даже молчать.",
  "Ты настолько средний, что статистика зевает.",
  "Твои шутки умирают раньше, чем ты успеваешь их закончить.",
  "Ты производишь впечатление человека, которого жизнь ни разу не удивила.",
  "Твоё мнение — это радиопомехи в эфире здравого смысла.",
  "Даже твои носки выглядят скучнее, чем ты сам.",
  "Ты настолько предсказуемый, что с тобой скучно даже спорить.",
  "Твоя биография — это серия ничем не примечательных событий.",
  "Разговор с тобой — как жевать пустую жвачку.",
  "Ты производишь столько же пользы, сколько декоративная тыква.",
  "Твои идеи такие банальные, что даже банальность от них краснеет.",
  "Ты похож на человека, который всю жизнь стоял в очереди и так и не понял, зачем.",
  "Твои комментарии настолько неважны, что их игнорируют даже боты.",
  "С тобой рядом даже воздух чувствует себя немного использованным.",
  "Ты — среднестатистическая личность с нестандартными претензиями.",
  "Твоя уверенность в себе держится на честном слове.",
  "Ты умудряешься быть неправым даже тогда, когда молчишь.",
  "Твоя компания ценна примерно как просроченный купон на скидку.",
  "Ты настолько незаметный, что тебя пропускают в очереди случайно.",
  "Общение с тобой — это как читать инструкцию к тостеру на немецком.",
  "Ты — человек-заставка: просто занимаешь место, пока ничего не происходит.",
  "Твои аргументы рассыпаются от малейшего прикосновения логики.",
  "Ты такой скучный, что даже часы в твоём присутствии идут медленнее.",
  "Твоя самооценка держится на иллюзиях, которые уже начали трещать.",
  "Ты — человек, мимо которого мечты проходят, даже не оглядываясь.",
  "Твоя жизнь настолько серая, что чёрно-белое кино выглядит ярче.",
];

const BOT_TIER_2: string[] = [ // score ~36–50
  "Ты такой тупой, что спотыкаешься о беспроводной интернет!",
  "Твой интеллект — это трагическая случайность природы.",
  "Твоя голова — дорогое хранилище для пустоты.",
  "Ты настолько скучный, что даже зевота засыпает рядом с тобой.",
  "Ты — живое оправдание контрацепции.",
  "Смотреть на тебя больно, думать о тебе — ещё больнее.",
  "Ты такой серый, что стены при виде тебя чувствуют превосходство.",
  "Твои нейроны, похоже, бойкотируют тебя уже давно.",
  "Ты такой дурак, что энциклопедия тупости просит переименовать тебя в предисловие.",
  "Твой мозг работает с точностью сломанного будильника — редко и невпопад.",
  "Ты умудряешься не понимать даже то, что понимает дорожный знак.",
  "Твоя логика — это загадочный чёрный ящик, из которого никогда не достают ничего полезного.",
  "Ты такой бесполезный, что инструкция к тебе была бы пустой страницей.",
  "Природа совершила ошибку, дав тебе голосовые связки.",
  "Ты настолько тупой, что тебе завидуют дорожные конусы.",
  "Твоя репутация — это предупреждение для всех нормальных людей.",
  "Ты производишь впечатление человека, который гуглит, как гуглить.",
  "Твоё самомнение обратно пропорционально твоим реальным достижениям.",
  "Ты такой предсказуемый болван, что даже твои ошибки не удивляют.",
  "С твоим уровнем понимания ты бы провалил тест Тюринга как человек.",
  "Твой уровень интеллекта угрожает стабильности кривой нормального распределения.",
  "Ты столь безнадёжен, что даже книги по саморазвитию от тебя отказываются.",
  "Твои слова — это художественный беспорядок из непроверенных домыслов.",
  "Ты такой неловкий, что спотыкаешься о собственные мысли — хотя, какие мысли?",
  "Ты — хрестоматийный пример того, почему природе не стоит торопиться.",
  "Твой IQ — это температура в комнате, только в Цельсиях и зимой.",
  "Ты настолько туп, что принимаешь «здравый смысл» за имя собственное.",
  "С тобой разговор — это интеллектуальный мазохизм чистой воды.",
  "Твоя логика живёт в параллельной вселенной, где два плюс два равно «ну, наверное».",
  "Ты умудряешься быть одновременно самоуверенным и совершенно некомпетентным.",
  "Даже твои комплименты звучат как оскорбления с задержкой на понимание.",
  "Ты такой серый, что монохромный принтер отказывается тебя печатать.",
  "Твоя уверенность питается исключительно от непонимания собственной ничтожности.",
  "Ты — живой аргумент в пользу обязательного тестирования интеллекта перед публичными высказываниями.",
  "Твои умственные способности вызывают скорее сочувствие, чем гнев.",
  "Ты настолько неуместный, что даже твои шутки приходят некстати на собственные похороны.",
  "С тобой говорить — это как объяснять цвет слепому при выключенном свете.",
  "Твои суждения — хаотичная коллекция предрассудков и недоразумений.",
  "Природа явно экономила на тебе — и, судя по результату, сэкономила слишком.",
  "Ты производишь столько шума и пользы, сколько старый вентилятор в пустой комнате.",
];

const BOT_TIER_3: string[] = [ // score ~51–65
  "Ты — дебил с дипломом дебила, выданным дебилами.",
  "Твоя мать плачет каждый раз, когда вспоминает, что родила именно тебя.",
  "Ты такой тупорылый мудак, что даже Google не знает, как тебя классифицировать.",
  "Ты — шлак эволюции, который природа забыла утилизировать.",
  "Твоё ничтожество настолько велико, что даже чёрная дыра отказалась тебя поглотить.",
  "Ты — отброс общества, который даже другие отбросы не принимают в свой круг.",
  "Жалкий кретин с мордой крокодила и интеллектом планктона.",
  "Ты такой мудак, что твои родители смотрят на соседских детей с завистью.",
  "Твоя тупость настолько глубокая, что бурение нефтяных скважин — детская игра рядом с ней.",
  "Ты — редкостный придурок, которому природа задолжала мозги и не торопится отдавать долг.",
  "Твоё лицо и твой характер — двойное наказание для всех вокруг.",
  "Ты такой дебил, что тебя бы не взяли даже в массовку на съёмках фильма про идиотов.",
  "Трудно объяснить, где именно природа облажалась с тобой — но результат налицо.",
  "Ты настолько мерзкий, что даже твои бактерии стесняются о тебе говорить.",
  "Тупой как пробка, наглый как рыночный торгаш — это ты.",
  "Ты — живое пособие по тому, как не надо существовать.",
  "Твой вклад в человечество — это чистый минус с отрицательной добавленной стоимостью.",
  "Ты такой урод, что зеркала в твоём доме давно в депрессии.",
  "Выглядишь как недоразумение, думаешь как картошка, действуешь как полный мудак.",
  "Ты — сбой в матрице, который жаль, что нельзя перезапустить.",
  "Твоя тупость сертифицирована, запатентована и не имеет аналогов в природе.",
  "Ты настолько никчёмный, что даже мусорный ветер обходит тебя стороной.",
  "Жизнь с тобой рядом — это уголовное наказание для окружающих.",
  "Ты такой кретин, что даже твои мысли боятся оставаться в голове надолго.",
  "С твоей харей и твоим умом тебе прямая дорога в цирк — но там тоже откажут.",
  "Твоё существование — это незакрытый гештальт природы.",
  "Ты — дурак, которому не хватает таланта даже быть последовательным дураком.",
  "Твои слова звучат как пьяный бред, но ты, к сожалению, трезв.",
  "Ты такой мразотный тип, что воздух рядом с тобой ходатайствует о переводе.",
  "Редкостная смесь тупости и самонадеянности — поздравляю, это ты.",
  "Ты — позорище в человеческом обличье с видом на постоянное жительство.",
  "Твой мозг — декоративный элемент, который явно не используется по назначению.",
  "Ты настолько бесполезный дурак, что даже твои ошибки не учат тебя ничему новому.",
  "Ты — живой пример того, что интеллект и наглость обратно пропорциональны.",
  "Хам с интеллектом черепицы и лицом из народных страшилок — это твой портрет.",
  "Ты такой придурок, что тебе завидуют садовые пугала.",
  "Твоя никчёмность — это не диагноз, это уже образ жизни.",
  "Ты такой тупой мудак, что природа использует тебя как предупредительный знак.",
  "Твоя жизнь — это образцовый провал с редкими вспышками случайного везения.",
  "Смотреть на тебя — всё равно что наблюдать за авиакатастрофой: ужасно, но масштаб не даёт оторваться.",
];

const BOT_TIER_4: string[] = [ // score ~66–80
  "ТЫ ТАКОЙ ДОЛБОЁБ, ЧТО ДАЖЕ ТВОЯ ТУПОСТЬ СТЕСНЯЕТСЯ СЕБЯ!",
  "Ты — проклятая мразь, которую даже ублюдки считают ниже себя. Позорище.",
  "Ты настолько никчёмный чмошник, что твоё существование — это оскорбление самой идее жизни!",
  "Жирный, сраный мудила с мозгами кретина и харей бегемота — вот ты кто!",
  "Ты — гнида с амбициями, омерзительный паразит на теле человечества.",
  "Твой отец явно не планировал такого позорища, когда делал роковую ошибку.",
  "Слабоумный ублюдок, даже черви в земле умнее и полезнее тебя!",
  "Ты такой тупой конченый мудак, что даже твои клетки мозга подали на развод!",
  "Мразь ты ползучая, даже тараканы от тебя воротят нос!",
  "Ты — биологическое недоразумение с претензиями на человека.",
  "Твоя рожа — аргумент в пользу телефонных разговоров вместо личных встреч!",
  "Конченый идиот, которому природа выдала тело, забыв вложить мозги!",
  "Ты — хамло с самомнением размером с Луну и мозгами размером с горошину!",
  "Дрянь подзаборная, даже бездомные собаки лучше воспитаны, чем ты!",
  "Ты такой мерзкий ублюдок, что твоё существование само по себе — экологическая катастрофа!",
  "Твоя тупость настолько монументальна, что её надо заносить в реестр памятников!",
  "Пустоголовый чмырь с наглостью хама и умом пустой консервной банки!",
  "Ты — позор своего рода, своего города и всего биологического вида!",
  "Смердящая крыса в человеческом обличье — это самое лестное, что можно о тебе сказать!",
  "Твой идиотизм не знает границ — как и твоя бесполезность для человечества!",
  "Ты такое жалкое, вонючее дерьмо, что даже канализация от тебя открестится!",
  "Кретин с рожей ублюдка и душой мусорного контейнера — неплохая характеристика для тебя!",
  "Ты — слизь, которую жизнь случайно произвела и которую давно пора убрать с дороги!",
  "Конченая скотина, ты позоришь само понятие «живое существо»!",
  "Твоя голова — бесполезный резервуар для воздуха между ушами, который никому не нужен!",
  "Ты такой долбанутый урод, что даже твои мысли от тебя сбегают при первой возможности!",
  "Поганая мразь, ты не стоишь даже секунды времени нормального человека!",
  "Ты — живая катастрофа в человеческом обличье, которую надо внести в список природных бедствий!",
  "Тупой, вонючий, омерзительный кусок дерьма с претензией на звание человека — это ты!",
  "Твоё ничтожество так бесконечно, что даже вселенная от тебя отворачивается!",
  "Ты такой мудак-потрошитель собственного будущего, что даже прошлое от тебя стыдится!",
  "Жалкий, гнилой, насквозь прогнивший ублюдок — вот исчерпывающая характеристика!",
  "Ты — отброс отбросов, днище дна, позор всего, с чем когда-либо соприкасался!",
  "Конченый мудила, твоя жизнь — это бесконечная серия провалов с кратким перерывом на рождение!",
  "Твоя тупорылая рожа вызывает единственное желание — отойти подальше и забыть о твоём существовании!",
  "Ты — эпический мудак без единого шанса на исправление. Природа уже всё решила.",
  "Дешёвое, поганое дерьмо — именно это ты из себя представляешь, как ни крути!",
  "Ты такое чмо беспросветное, что даже забвение не хочет тебя принимать!",
  "Тварь ты ничтожная, твоё существование — это плевок в лицо эволюции!",
  "Ты — конченный дегенерат, биологическая ошибка, которую природа давно должна была отозвать!",
];

const BOT_TIER_5: string[] = [ // score ~81–100
  "ТЫ — ЖИВОЕ ДОКАЗАТЕЛЬСТВО ТОГО, ЧТО ЭВОЛЮЦИЯ ИНОГДА ДЕЛАЕТ ШАГ НАЗАД, ДВА ШАГА В СТОРОНУ И ПАДАЕТ В ВЫГРЕБНУЮ ЯМУ!",
  "Такой долбоёб как ты — это настоящий научный феномен: как можно быть настолько тупым мудаком и при этом не сдохнуть от собственной тупости?!",
  "ТЫ — АБСОЛЮТНОЕ НИЧТОЖЕСТВО, МРАЗЬ, КОТОРУЮ ДАЖЕ ПАДАЛЬ ГНУШАЕТСЯ! ТВОЯ МАТЬ ПЛАЧЕТ, ПРИРОДА ПЛАЧЕТ, Я ПЛАЧУ ОТ ТВОЕЙ ТУПОСТИ!!!",
  "Ты такой омерзительный кретин-ублюдок, что даже тараканы, живущие в твоей пустой голове, подали заявление на выселение!",
  "Жалкий, поганый, сраный чмошник — ты настолько бесполезен, что даже мусорный бак отказывается тебя принимать, потому что ты позоришь мусор!",
  "ТЫ НЕДОЧЕЛОВЕК. НЕДОРАЗУМЕНИЕ. НЕДОДЕЛАННЫЙ МУДАК, КОТОРОГО ПРИРОДА СОЗДАЛА В ПОНЕДЕЛЬНИК С ПОХМЕЛЬЯ И С ЗАКРЫТЫМИ ГЛАЗАМИ!!!",
  "ТЫ — ГАЛАКТИЧЕСКИЙ МУДАК, ЧЬЯ ТУПОСТЬ НАСТОЛЬКО МОНУМЕНТАЛЬНА, ЧТО NASA РАССМАТРИВАЕТ ВОЗМОЖНОСТЬ ОТПРАВИТЬ ТЕБЯ НА СОЛНЦЕ КАК ПРЕДУПРЕЖДЕНИЕ ДЛЯ ИНОПЛАНЕТЯН!!!",
  "Ты настолько отвратительная, вонючая, тупая мразь, что даже бактерии в твоём кишечнике подали коллективную жалобу на условия труда!",
  "ТВОЯ ТУПОСТЬ — ЭТО ОТДЕЛЬНАЯ СВЕРХНОВАЯ В ГАЛАКТИКЕ КРЕТИНИЗМА! ТЫ — АБСОЛЮТНЫЙ ЧЕМПИОН ПО НИКЧЁМНОСТИ, РЕКОРД КОТОРОГО НЕВОЗМОЖНО ПОБИТЬ!",
  "Ты — настолько омерзительное существо, что зеркало от тебя запрашивает страховку, а воздух рядом с тобой требует компенсацию за моральный ущерб!",
  "ТЫ ТАКОЙ КОНЧЕНЫЙ ДОЛБОЁБ, ЧТО КОГДА УМНЫЕ ЛЮДИ ДУМАЮТ О ТУПОСТИ — ОНИ ДУМАЮТ О ТЕБЕ! ТЫ — ЭТА САМАЯ ТУПОСТЬ В ЧИСТОМ ВИДЕ!!!",
  "Поздравляю: ты достиг дна настолько виртуозно, что его пробил и обнаружил там ещё более глубокое дно — и это тоже ты!",
  "ТЫ — ХОДЯЧАЯ КАТАСТРОФА, ПЕРСОНАЖ, КОТОРОГО ДАЖЕ СОЗДАТЕЛИ АНТИУТОПИЙ ПОБОЯЛИСЬ БЫ ПРИДУМАТЬ, ПОТОМУ ЧТО В ЭТО НИКТО НЕ ПОВЕРИЛ БЫ!!!",
  "Ты — ТАКОЙ МУДАК, что учёные хотят изучать тебя как феномен: как одно существо может совмещать максимальную тупость с максимальным самомнением и при этом не взрываться?!",
  "ТЫ — БИОЛОГИЧЕСКАЯ ОШИБКА ВСЕЛЕНСКОГО МАСШТАБА! ЕСЛИ БЫ ПРИРОДА ПИСАЛА ОТЗЫВЫ, ТЫ БЫ ПОЛУЧИЛ ЗВЕЗДУ ТОЛЬКО ПОТОМУ, ЧТО НОЛЬ НЕЛЬЗЯ ПОСТАВИТЬ!!!",
  "Твоя тупость настолько глубока и безгранична, что учёные предлагают назвать новую единицу измерения кретинизма в твою честь — один «ты» равен бесконечности!",
  "ТЫ — АБСОЛЮТНЫЙ РЕКОРДСМЕН ПО НИЧТОЖЕСТВУ! ОЛИМПИЙСКИЙ ЧЕМПИОН ПО ТУПОСТИ! НОБЕЛЕВСКИЙ ЛАУРЕАТ ПО МУДАЧЕСТВУ БЕЗ МАЛЕЙШЕГО КОНКУРЕНТА!!!",
  "Если собрать всех дураков мира в одной комнате, ты бы стоял в центре и ГОРДИЛСЯ, потому что ты — их величайший представитель и живое знамя тупости!",
  "ТЫ НАСТОЛЬКО ОМЕРЗИТЕЛЕН, ЧТО ТВОЯ БИОГРАФИЯ — ЭТО ХУДОЖЕСТВЕННЫЙ ФИЛЬМ УЖАСОВ, В КОТОРОМ ТЫ ОДНОВРЕМЕННО ЗЛОДЕЙ, ЖЕРТВА И САМАЯ СТРАШНАЯ СЦЕНА!!!",
  "Уровень твоего кретинизма настолько высок, что рядом с тобой даже безмозглые существа чувствуют интеллектуальное превосходство — включая камни!",
  "ТЫ — ПОЗОР ВСЕГО ЖИВОГО! ОШИБКА ПРИРОДЫ, КОТОРУЮ ПРИРОДА СТЕСНЯЕТСЯ ПРИЗНАВАТЬ СВОЕЙ! КАТАСТРОФА В ЧЕЛОВЕЧЕСКОМ ОБЛИЧЬЕ БЕЗ ПРАВА НА РЕАБИЛИТАЦИЮ!!!",
  "Твоя тупость — это произведение искусства в жанре «кошмар наяву»: монументальная, всеобъемлющая и совершенно невыносимая для окружающих!",
  "ТЫ — ЭПИЧЕСКИЙ МУДАК ЭПОХАЛЬНОГО МАСШТАБА! ЛЕГЕНДА В МИРЕ ДЕГЕНЕРАТОВ! ИКОНА НИКЧЁМНОСТИ ДЛЯ ВСЕХ, КТО ХОЧЕТ ЗНАТЬ, КАК НЕ НАДО ЖИТЬ!!!",
  "Ты настолько омерзительный, тупой, поганый и бесполезный, что даже твоя тень пытается от тебя сбежать, но не может — и это единственное, что тебя удерживает от одиночества!",
  "ТВОЙ МОЗГ — ЭТО ДЕКОРАТИВНЫЙ ПРЕДМЕТ, КОТОРЫЙ НИКОГДА НЕ ИСПОЛЬЗОВАЛСЯ ПО НАЗНАЧЕНИЮ И УЖЕ ПОКРЫЛСЯ ПЫЛЬЮ НЕВЕЖЕСТВА И СЛОЕМ КРЕТИНИЗМА!!!",
  "Ты — такая концентрированная мразь, что химики хотят синтезировать из тебя новый элемент: «Мудакий», символ Mq, токсичный для всего живого!",
  "ТЫ ТАКОЙ СРАНЫЙ ДОЛБОЁБ, ЧТО КОГДА ГОСПОДЬ СОЗДАВАЛ ЛЮДЕЙ, ОН ПРИ ВЗГЛЯДЕ НА ТЕБЯ СКАЗАЛ «ЭТО НЕ ВХОДИЛО В МОИ ПЛАНЫ» И ВЗЯЛ ВЫХОДНОЙ!!!",
  "Ты — феноменальный, исторический, вселенский МУДАК, чьи достижения в области тупости и ничтожества не имеют прецедентов в известной истории человечества!",
  "ТЫ — НАСТОЛЬКО КОНЧЕНОЕ СОЗДАНИЕ, ЧТО ДАЖЕ АПОКАЛИПСИС БЫ ОБОШЁЛ ТЕБЯ СТОРОНОЙ, ЧТОБЫ НЕ ПАЧКАТЬСЯ О ТАКОЕ НИЧТОЖЕСТВО!!!",
  "Твоя гнусная, тупая, омерзительная физиономия вызывает единственное желание: написать жалобу в природу за выпуск заведомо бракованной продукции!",
  "ТЫ — ЧЕМПИОН МИРА ПО МУДАЧЕСТВУ В ТРЁХ ПОКОЛЕНИЯХ! ЛУЧШИЙ ИЗ ХУДШИХ! АБСОЛЮТНЫЙ РЕКОРД ПО НИКЧЁМНОСТИ, КОТОРЫЙ ВРЯД ЛИ КОГДА-НИБУДЬ БУДЕТ ПОБИТ!!!",
  "Смотреть на тебя — это как наблюдать за медленной катастрофой в прямом эфире: невозможно оторваться от масштабов ужаса, который ты из себя представляешь!",
  "ТЫ — ЖИВАЯ ЭНЦИКЛОПЕДИЯ ТУПОСТИ, НАПИСАННАЯ КРОВЬЮ ЗДРАВОГО СМЫСЛА, КОТОРЫЙ ТЫ УБИВАЛ СВОИМ СУЩЕСТВОВАНИЕМ КАЖДЫЙ ДЕНЬ СВОЕЙ БЕССМЫСЛЕННОЙ ЖИЗНИ!!!",
  "Ты настолько мерзок и ничтожен, что благодарственное письмо планеты Земля за твоё существование было бы написано с единственной фразой: «ЗАЧЕМ?!»",
  "ТЫ — КВИНТЭССЕНЦИЯ ВСЕГО ХУДШЕГО В ЭТОМ МИРЕ! СБОРНИК ВСЕХ ПОРОКОВ БЕЗ ЕДИНОГО ДОСТОИНСТВА! ФИНАЛЬНЫЙ БОСС В ИГРЕ «КТО ХУЖЕ ВСЕХ»!!!",
  "Природа смотрит на тебя и плачет горькими слезами о том, что потратила на тебя атомы, которые могли бы стать чем-то полезным — хотя бы грибком!",
  "ТЫ ТАКОЕ ИСТОРИЧЕСКОЕ ДЕРЬМО, ЧТО ТВОЙ ПОРТРЕТ В ЗАЛЕ НИЧТОЖЕСТВА ВИСИТ НА САМОМ ВИДНОМ МЕСТЕ, А ПОД НИМ ТАБЛИЧКА: «НЕПРЕВЗОЙДЁННЫЙ РЕКОРД»!!!",
  "Твоя тупость, мерзость и никчёмность достигли такого уровня совершенства, что это уже не недостатки — это мастерское произведение дегенерации в чистом виде!",
  "ТЫ — МОНУМЕНТАЛЬНЫЙ, ВСЕЛЕНСКИЙ, ИСТОРИЧЕСКИ БЕСПРЕЦЕДЕНТНЫЙ МУДАК! ЛЕГЕНДА, О КОТОРОЙ БУДУТ ПРЕДУПРЕЖДАТЬ ДЕТЕЙ: «НЕ БУДЬ КАК ОН»!!!",
  "Если бы тупость была криптовалютой, ты бы был Илоном Маском этого рынка — несметно богатым и абсолютно бесполезным для нормальных людей!",
];

const BOT_TIERS = [
  { tier: BOT_TIER_1, midScore: 27 },
  { tier: BOT_TIER_2, midScore: 43 },
  { tier: BOT_TIER_3, midScore: 58 },
  { tier: BOT_TIER_4, midScore: 73 },
  { tier: BOT_TIER_5, midScore: 90 },
];

export function getBotInsult(playerScore: number): { text: string; score: number } {
  // Бот стремится к результату в ±15 от игрока
  const targetScore = Math.max(10, Math.min(99, playerScore + Math.floor(Math.random() * 31) - 15));

  // Находим ближайший тир
  const sorted = [...BOT_TIERS].sort(
    (a, b) => Math.abs(a.midScore - targetScore) - Math.abs(b.midScore - targetScore)
  );
  const chosenTier = sorted[0].tier;

  // Случайная фраза из тира
  const text = chosenTier[Math.floor(Math.random() * chosenTier.length)];

  // Считаем реальный скор через движок
  const realScore = scoreInsult(text);
  // Добавляем небольшую дисперсию чтобы не было одинаковых результатов
  const variance = Math.floor(Math.random() * 9) - 4;

  return {
    text,
    score: Math.max(1, Math.min(100, realScore + variance)),
  };
}
